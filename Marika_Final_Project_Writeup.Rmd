...
```{r}
shapefiles <- readOGR("Data/ZIP_CODE_040114") %>% spTransform(CRS("+proj=longlat +datum=WGS84"))
shapefiles.f <- shapefiles %>% fortify(region = 'ZIPCODE')
shapefiles.f$id <- as.factor(shapefiles.f$id)
plotting_zips1 <- merge(census, shapefiles.f, by.x = "ZIP", by.y = "id", all.x = TRUE)
write.csv(plotting_zips1, file = "Data/NY_shapefiles.csv")
```
